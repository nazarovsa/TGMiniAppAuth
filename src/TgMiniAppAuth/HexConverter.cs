...
{
  // https://github.com/dotnet/runtime/blob/5e1081f6a0c86489e7fda575735ef49ff2ca3efa/src/libraries/Common/src/System/HexConverter.cs
 ...
 Vector128.LoadUnsafe(ref srcRef, offset + (nuint)Vector128<ushort>.Count).AsUInt16();
 ...
 PackedSimd.BitwiseSelect(nibbles, shiftedNibbles, Vector128.Create<ushort>(0xFF00).AsByte());
 ...
 Vector128.Shuffle(output, Vector128.Create((byte)0, 2, 4, 6, 8, 10, 12, 14, 0, 0, 0, 0, 0, 0, 0, 0));
            ...
            {
                fallbackResult =
 TryDecodeFromUtf8_Scalar(Unsafe.BitCast<ReadOnlySpan<TChar>, ReadOnlySpan<byte>>(source.Slice((int)offset)), destination.Slice((int)(offset / 2)), out elementsProcessed);
            }
            else
            {
                Debug.Assert(typeof(TChar) == typeof(ushort));
                fallbackResult =
 TryDecodeFromUtf16_Scalar(Unsafe.BitCast<ReadOnlySpan<TChar>, ReadOnlySpan<char>>(source.Slice((int)offset)), destination.Slice((int)(offset / 2)), out elementsProcessed);
            }
    ...
    private static bool TryDecodeFromUtf8_Scalar(ReadOnlySpan<byte> utf8Source, Span<byte> destination,
      out int bytesProcessed)
    {
      Debug.Assert((utf8Source.Length % 2) == 0, "Un-even number of characters provided");
      Debug.Assert((utf8Source.Length / 2) == destination.Length,
    ...
    private static bool TryDecodeFromUtf16_Scalar(ReadOnlySpan<char> source, Span<byte> destination,
