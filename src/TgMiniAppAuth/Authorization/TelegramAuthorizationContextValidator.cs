...
internal static class TelegramAuthorizationContextValidator
{
  ...
  internal static bool IsValidTelegramMiniAppContext(string urlEncodedString,
    string token,
    out DateTimeOffset issuedAt, int? stackAllocationThreshold = null)
  {
    ...
    Span<char> ordered = stackalloc char[decodedLength];
    ...
    CopyOrderedBuffer(decodedBuffer, pairsBorders, ordered, newPairBorders, threshold);
    ...
    Span<char> miniAppCheckDataSpan = ordered.Length <= threshold
      ? stackalloc char[threshold]
      : new char[ordered.Length];
    ordered.CopyTo(miniAppCheckDataSpan);

    Span<byte> checkDataBytes = ordered.Length <= threshold
      ? stackalloc byte[threshold]
      : new byte[ordered.Length];
    ...
    int? stackAllocationThreshold = null)
  {
    ...
    for (int pos = 0; pos < count; pos++)
    {
      ...
      else if (ch == '%' && pos < count - 2)
      {
        ...
        {
          int h1 = HexConverter.FromChar(input[pos + 2]);
          int h2 = HexConverter.FromChar(input[pos + 3]);
          int h3 = HexConverter.FromChar(input[pos + 4]);
          int h4 = HexConverter.FromChar(input[pos + 5]);
        ...
        {
          int h1 = HexConverter.FromChar(input[pos + 1]);
          int h2 = HexConverter.FromChar(input[pos + 2]);
